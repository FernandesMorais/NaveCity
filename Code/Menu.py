...
class Menu:
    ...
    def run(self, menu_option=0):
        pygame.mixer_music.load('./assets/musicamenu.wav')
        pygame.mixer_music.play(-1)
        while True:
            self.window.blit(self.surf, self.rect)
            self.menu_text(text_size=50, text="AERO", text_color=COLOR_WHITE,text_center_pos=((WIN_WIDTH / 2), 70))
            self.menu_text(text_size=50, text="CITY", text_color=COLOR_WHITE, text_center_pos=((WIN_WIDTH / 2), 120))
